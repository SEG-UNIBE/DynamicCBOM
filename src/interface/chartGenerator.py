...
class ChartGenerator:
    ...
    def _parse_cbom(self, cbom_path: str) -> list:
        ...
        with open(cbom_path, 'r') as f:
            cbom = json.load(f)
        cbom = [
            component for component in cbom.get("components", [])
    ...
    def _compare_with_ground_truth(self, cbom: list, gt: list) -> tuple:
        ...
        """
        matcher = CBOMMatcher()
        _, precision, recall, f1_score = matcher.match_assets(
            gt=gt,
            target=cbom,
            threshold=0.6
        )
    ...
    def _filter_assets_by_primitive(self, cbom: list, primitives: list) -> list:
        ...
        """
        return [
            asset for asset in cbom
            if asset.get('cryptoProperties', {}).get('algorithmProperties', {}).get('primitive') in primitives
        ]
    ...
    def _get_asymmetric_assets(self, cbom: list) -> list:
        ...
        """
        return self._filter_assets_by_primitive(
            cbom, settings.primitive_mapping.asymmetric
        )
    ...
    def _get_symmetric_assets(self, cbom: list) -> list:
        ...
        """
        return self._filter_assets_by_primitive(
            cbom, settings.primitive_mapping.symmetric
        )
    ...
    def _get_hashing_assets(self, cbom: list) -> list:
        ...
        """
        return self._filter_assets_by_primitive(
            cbom, settings.primitive_mapping.hashing
        )
    ...
    def _calculate_singular_metrics(self, gt_cbom: list, dyn_cbom: list) -> dict:
        ...
        categories = {
            ...
            'asymmetric': (self._get_asymmetric_assets(dyn_cbom), self._get_asymmetric_assets(gt_cbom)),
            'symmetric': (self._get_symmetric_assets(dyn_cbom), self._get_symmetric_assets(gt_cbom)),
    ...
    def _configure_axis(self, ax) -> None:
        ...
        """
        ax.grid(True, axis="y", which="major", linestyle="--", linewidth=0.5, color="0.75", alpha=0.8)
        ax.grid(True, axis="x", which="major", linestyle="--", linewidth=0.5, color="0.85", alpha=0.8)
    ...
    def generate_comparisons(
        ...
        self._plot_comparison_of_two(
            np.array(dyn_prec), np.array(ibm_prec),
            ax[0, 1], ["Overall", "Asym", "Sym", "Hash"], "Precision (%)"
        )

        # Panel 3: Recall comparison
        self._plot_comparison_of_two(
            np.array(dyn_recall), np.array(ibm_recall),
            ax[1, 0], ["Overall", "Asym", "Sym", "Hash"], "Recall (%)"
        )

        # Panel 4: F1-score comparison
        self._plot_comparison_of_two(
            np.array(dyn_f1), np.array(ibm_f1),
            ax[1, 1], ["Overall", "Asym", "Sym", "Hash"], "F1-score (%)"
        )
    ...
    def _plot_comparison_of_two(
        self, dyn: np.ndarray, ibm: np.ndarray,
        ax, labels: list, ylabel: str
    ) -> None:
    ...
    def _plot_comparison_of_three(
        self, gt: np.ndarray, dyn: np.ndarray, ibm: np.ndarray,
        ax, labels: list, ylabel: str
    ) -> None:
